...
rollingTimeseries = []
from numpy import floor
for i in xrange(int(floor((len(words)-10000)/1000))):
  chunk = ''
  if i == int(floor((len(words)-10000)/1000))-1:
    # take the rest
    print 'last chunk'
    print 'getting words ' + str(i*1000+10000) + ' through ' + str(len(words)-1)
    for j in xrange(i*1000+10000,len(words)-1):
      chunk += words[j]+' '
  else:
    print 'getting words ' + str(i*1000) + ' through ' + str(i*1000+10000)
    for j in xrange(i*1000,10000+i*1000):
      chunk += words[j]+' '
  print chunk[0:10]
  textValence,textFvec = emotion(chunk,labMT,shift=True,happsList=labMTvector)
  print 'the valence of {0} part {1} is {2}'.format('moby_dick',i,textValence)
